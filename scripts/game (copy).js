...
var ballDirX = 1, ballDirY = 1, ballSpeed = 2.1;
...
function init()
{
	...
	// We Want the ball ot be a bit above the table's ground, keeping in mind of the radius we have set.
	ball.position.z = 15;
...
function ballPhysics()
{
	...
	if (ball.position.x <= -fieldWidth/2 || ball.position.x >= fieldWidth/2) // if ball goes off and hit the Wall of the goalside(not goal but sides, which gives reflection)
	{	
		if ((ball.position.y >= -fieldHeight/4 + 10) && (ball.position.y <= fieldHeight/4 - 10)){ //IF a Goal iss scored. on either Side
				ball.position.x = 0;
				ball.position.y = 0;
				ballDirY = -ballDirY
				ballDirX = -ballDirX;
	...
	if (ball.position.y <= -fieldHeight/2 + 10 || ball.position.y >= fieldHeight/2 - 10)
	{
		ballDirY = -ballDirY;
		ball.rotation.y = -ball.rotation.y;
	}	
...
function paddlePhysics()
{
	...
	//ball.position.y is higher on left side of table, and negative towards rights side of table
	if (ball.position.y <= paddle1.position.y + paddleHeight/2 +30&&  ball.position.y >= paddle1.position.y - paddleHeight/2-30)
	{
